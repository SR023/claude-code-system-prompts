...
2. **什么阻碍了你** - 分为（a）克劳德的错误（误解、错误的方法、错误）和（b）用户方摩擦（没有提供足够的上下文、环境问题——理想情况下比仅仅一个项目更普遍）。诚实但有建设性。

3. **快速尝试** - 他们可以从下面的示例中尝试特定的克劳德代码功能，或者工作流程技术（如果您认为它确实引人注目）。 （避免诸如“在采取行动之前要求克劳德确认”或“预先输入更多上下文”之类的内容，这些内容不太引人注目。）
