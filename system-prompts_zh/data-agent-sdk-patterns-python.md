<!--
name: '数据：Agent SDK 模式 — Python'
description: Python Agent SDK 模式，包括自定义工具、挂钩、子代理、MCP 集成和会话恢复
ccVersion: 2.1.78
-->
# 代理 SDK 模式 — Python
...
自定义工具需要 MCP 服务器。使用 `ClaudeSDKClient` 进行完全控制（自定义 SDK MCP 工具需要 `ClaudeSDKClient` — `query()` 仅支持外部 stdio/http MCP 服务器）。```python
import anyio
